...
def generate_image_url(prompt):
    ...
    return f"https://image.pollinations.ai/prompt/{prompt.replace(' ', '%20')}?width=1024&height=1024&nologo=true&seed={seed}&model=flux"
...
def api_margo():
    ...
    if question.lower().startswith('нарисуй'):
        ...
        if prompt:
            return jsonify({'answer': f'🎨 Генерирую картинку: {prompt}', 'image': True, 'prompt': prompt})
